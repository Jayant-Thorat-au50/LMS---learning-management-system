...
function Header() {
  ...
  let [list, setList] = useState(coursesList.map(c => c.catagory == catagoryList[0]))
    ...
    return (
        <header className=" z-50 sticky top-0 left-0  shadow-[0_0_5px_gray] bg-blue-100 h-[11.5vh] w-full hidden lg:block ">
                  ...
                  <div className="  h-full px-16 flex w-full justify-between items-center text-2xl text-white">
                    <ul className=" flex justify-center  h-full items-center gap-5 relative ">
                      ...
                      <li
                        ...
                        {showCourseCatagoryList ? (<div className=" absolute w-[940px] h-[22rem] top-12 left-[6.9rem]  shadow-[0_0_5px_gray] my-1 rounded flex bg-white">
                          <div className={`w-[27%] border-black h-full grid-rows-${catagoryList.length} grid`}>
                            {catagoryList.map((c,idx) => <div onMouseEnter={() =>{ coursesBycatagory(c)
                               setViewAllVal(c)}} key={idx} className=" hover:bg-gray-300 transition-all ease-in-out duration-200 flex justify-center items-center  border-2 border-t-0 ">
                                ...
                                list.map(course =>
                                  <div
                                   ...
                                   className=" text-black h-16 flex items-center justify-start gap-4 font-semibold   m-4  border border-gray-200 bg-[#F3F4F6] rounded">
                                    <img key={course._id} src={course?.thumbnail?.secure_Url} className=" w-16 h-16  rounded" alt="" />
                                    <p key={course._id} className="capitalize">{course.title}</p>
                                  </div>)
                              }
                      ...
                      <li >
                        ...
                        className=" text-black text-lg font-semibold">Add Courses</p>
                      </li>
                    </ul>
                    ...
                    <ul className=" flex h-full items-center  text-black ">
                      <li className=" bg-gray-100 rounded-md relative text-end border   overflow-hidden w-64">
                        ...
                        <input type="text" className=" focus:outline-none text-lg py-1 w-[80%] text-black bg-white" placeholder="Enter course name..." />
                      </li>
